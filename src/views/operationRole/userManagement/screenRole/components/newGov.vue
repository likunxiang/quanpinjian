				...
				<el-row>
					<el-col :span="12">
						<el-form-item v-if="ruleForm.userId == undefined" label="账号名称" prop="nickName">
							<el-input v-model="ruleForm.nickName" placeholder="仅支持英文和数字" maxlength="30"
					...
					<el-col :span="12">
						<el-form-item label="状态" prop="status">
							<el-radio-group v-model="ruleForm.status">
								<el-radio v-for="dict in statusList" :key="dict.value"
									:label="dict.value">
									{{dict.label}}
								</el-radio>
							</el-radio-group>
	...
	export default {
		...
		data() {
			return {
				...
				ruleForm: {
					nickName: '',
					deptId: 'c4f05fc3-f470-11ec-bace-0242ac120003',
					password: '',
					name: '',
					userTel: '',
					status: '0'
		...
		methods: {
			...
			async getEncryptStr(password) {
				...
				if (oldForm.password != ruleForm.password || oldForm.name != ruleForm.name || oldForm.userTel !=
					ruleForm.userTel || oldForm.status != ruleForm.status || oldForm.deptId != ruleForm.deptId) {
			...
			async getUserDetail() {
				...
				getRoleUserDetail({
					...
					this.ruleForm.status = data.accountStatus
					this.ruleForm.roleKey = 'sceneRole'
					...
					this.oldForm.password = ''
					this.oldForm.userTel = data.phonenumber
					this.oldForm.status = data.accountStatus
					this.oldForm.roleKey = 'sceneRole'
