...
<template>
  <el-dialog title="个人账号信息" :visible.sync="isOpen" width="700px" @close="beforeClose" append-to-body>
    <el-row class="">
      <div class="title-bg mb10">账号信息</div>
    ...
    <el-row>
      ...
      <el-row class="mb10">
        ...
        <div class="flex mb10">
          <el-image :preview-src-list="userInfo.imgArr" class="mr10" style="width: 100px; height: 100px" :src="imgs" fit="fit" v-for="(imgs,mIndex) in userInfo.imgArr" :key="mIndex"></el-image>
        </div>
      </el-row>
    </el-row>
    <el-row class="mb20">
  ...
  export default {
    ...
    methods: {
      ...
      async getUserInfoOne() {
        await getUserInfoOne(this.row.userId || this.userId).then((res) =>{
          if (res.Tag.length) {
            let data = res.Tag[0].Table[0]
