		...
		methods: {
			...
			checkPlate(index) {
				...
				if (code.length > 0) {
					...
					// 否则
					this.existPlateField(code, source, index)
				}

			},
			// 添加字段时，先判断是否重复添加
			existPlateField(code, source, index) {
				this.addPlateFieldFromSupply(code, source, index)
				...
				// 		this.addPlateFieldFromSupply(id, source, index)

				// 	}
				// })
			},
			async addPlateFieldFromSupply(id, source, index) {
				await addPlateFieldFromSupply({
					plateFieldGuid: this.fieldList[index].plateFieldGuid,
					plateFieldName: this.fieldList[index].plateFieldName || '',
					fixedDataCode: this.fieldList[index].plateFieldCode || '',
					curUserId: this.$store.state.user.adminId,
				}).then(res => {
					if (res.Tag[0] > 0) {
						this.$message({
			...
			async getRequireFieldsOfSupply(id, source, index, code) {
				await getRequireFieldsOfSupply({
					...
					if (res.Tag.length) {
						...
						console.log('匹配', data)
						this.$alert('【' + data[0].demandName + '】' + '在供应需求信息中对应的' + '【' + showField +
							'】,请确认添加', '', {
