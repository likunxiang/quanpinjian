	...
	<div class="app-container">
		...
		<el-table v-if="refreshTable" v-loading="loading" :data="menuList" row-key="menuId"
			...
			<el-table-column prop="status" label="状态" width="80">
				<template slot-scope="scope">
					<div>{{scope.row.status === '0'?'正常':'停用'}}</div>
				</template>
			</el-table-column>
		...
		methods: {
			...
			getList() {
				...
				listMenu(this.queryParams).then(response => {
					this.menuList = this.handleTree(response.Tag, "menuId");
					this.loading = false;
				});
			},
			/** 转换菜单数据结构 */
			...
			getTreeselect() {
				listMenu().then(response => {
					...
					menu.children = this.handleTree(response.Tag, "menuId");
			...
			handleUpdate(row) {
				...
				getMenu(row.menuId).then(response => {
					console.log('当前菜单',response);
					this.form = response.Tag;
					this.open = true;
					this.title = "修改菜单";
				});
			},
