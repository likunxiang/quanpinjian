<template>
  <div class="page">
    <div class="flex flex-center mb20">
      <el-input placeholder="请输入你要查找的采购编号" prefix-icon clearable v-model="searchVal"
    ...
    <arbitrationApply v-if="isArbitrationApply" @close="closeCheckArbitration" :openRow="openRow" :type="openRow.applyType"></arbitrationApply>
    <findingResultNotSys v-if="isFinding" @close="closeFindingDetail" :row="openRow" @refresh='getNonSysWaitJudgeList'></findingResultNotSys>
    <userMessage v-if="isDemandAccount" @close="closeDemandAccount" :row="openRow" :userId="openRow.demandUserId"></userMessage>
    <userMessage v-if="isApply" @close="closeCheckAccount" :row="openRow" :userId="openRow.applyUserId"></userMessage>
    <userMessage v-if="isSupplyAccount" @close="closeSupplyAccount" :row="openRow" :userId="openRow.supplyUserId"></userMessage>
...
<script>
  ...
  import findingResultNotSys from '@/views/arbitrationRole/ReturnOfRuling/components/findingResultNotSys.vue'
  ...
  import { getNonSysWaitJudgeList } from '@/api/arbitrationRoleApi/ReturnOfRuling.js'
  ...
  export default {
    ...
    methods: {
      // 获取数据
      async getNonSysWaitJudgeList() {
		this.loading = true
        await getNonSysWaitJudgeList({
